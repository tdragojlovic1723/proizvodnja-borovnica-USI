    ...
    <div class="bg-borovnica-light min-h-screen py-10">
        <div class="max-w-3xl mx-auto px-4">
            <div class="flex flex-col items-center mb-10">
                <div class="text-white font-bold italic text-3xl uppercase tracking-widest opacity-90">Izmena Skladišta</div>
            ...
            <div class="bg-borovnica-table rounded-sm shadow-2xl p-8 border border-borovnica-dark/20 text-borovnica-dark font-semibold">
                ...
                @endif
                <form action="{{ route('skladiste.update', $skladiste->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <input type="text" name="lokacija" value="{{ $skladiste->lokacija }}" class="w-full bg-white/50 border-borovnica-dark/20 rounded py-3" placeholder="npr. Kragujevac, Industrijska zona">
                        </div>

                        <div>
                            <label class="block font-bold italic uppercase text-sm mb-2">Kapacitet</label>
                            <input type="number" name="kapacitet" value="{{ $skladiste->kapacitet }}" class="w-full bg-white/50 border-borovnica-dark/20 rounded py-3">
                        </div>

                        <div>
                            <label class="block font-bold italic uppercase text-sm mb-2">Temperatura (C)</label>
                            <input type="number" step="0.01" name="temperatura" value="{{ $skladiste->temperatura }}" class="w-full bg-white/50 border-borovnica-dark/20 rounded py-3">
                        </div>

                        <div>
                            <label class="block font-bold italic uppercase text-sm mb-2">Dnevni trošak</label>
                            <input type="number" step="0.01" name="trosak" value="{{ $skladiste->trosak }}" class="w-full bg-white/50 border-borovnica-dark/20 rounded py-3">
                        </div>
                    </div>
